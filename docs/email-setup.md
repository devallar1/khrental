...
This document explains how to set up email sending for KH Rentals using Twilio SendGrid and the server-side `/api/send-email` endpoint.

## Why Use the Server Email Endpoint?

When sending emails from a web application, you should never call external APIs like Twilio SendGrid directly from the frontend because:

1. It would expose your API keys to the client-side code (a security risk)
2. Many API providers (including Twilio SendGrid) block browser-based calls due to CORS restrictions

Instead, the application uses the backend email endpoint to create a secure server-side hop that makes the Twilio SendGrid API call with proper authentication.

## Setup Instructions

### 1. Set Up a Twilio SendGrid Account

1. Sign up for Twilio SendGrid at [sendgrid.com](https://sendgrid.com/)
...
#### Twilio SendGrid API Errors
- If you see errors from the Twilio SendGrid API, check the error message and ensure your API key has proper permissions.
